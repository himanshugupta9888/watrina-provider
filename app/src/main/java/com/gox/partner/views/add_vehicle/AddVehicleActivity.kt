...
class AddVehicleActivity : BaseActivity<ActivityAddVehicleBinding>(), AddVehicleNavigator {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        EasyImage.handleActivityResult(requestCode, resultCode, data, this, object : DefaultCallback() {
            override fun onImagesPicked(
                ...
                //mViewModel.setRcBookUri(Uri.fromFile(imageFiles[0]))

                startCropImageActivity(Uri.fromFile(imageFiles[0]))

                if (Enums.RC_VEHICLE_IMAGE == reqCode) {
                    mViewModel.setVehicleUri(Uri.fromFile(imageFiles[0]))
                    Glide.with(applicationContext).load(imageFiles[0])
                        .placeholder(R.drawable.ic_car_placeholder)
                        .circleCrop().into(iv_vehicle)
                } else if (Enums.RC_RC_BOOK_IMAGE == reqCode) {
                    mViewModel.setRcBookUri(Uri.fromFile(imageFiles[0]))
                    Glide.with(applicationContext).load(imageFiles[0])
                        .into(iv_rc_book)
                    tvRcBook.visibility = View.GONE
                } else if (Enums.RC_INSURANCE_IMAGE == reqCode) {
                    mViewModel.setInsuranceUri(Uri.fromFile(imageFiles[0]))
                    Glide.with(applicationContext).load(imageFiles[0])
                        .into(iv_insurance)
        ...
        when (requestCode) {
            CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE -> {
                val result = CropImage.getActivityResult(data)
                if (resultCode == Activity.RESULT_OK) when (this.reqCode) {
    ...
    override fun onVehicleImageClicked() {
        if (getPermissionUtil().hasPermission(this, permissions)) {
            reqCode = Enums.RC_VEHICLE_IMAGE
    ...
    override fun onRcBookClicked() {
        if (getPermissionUtil().hasPermission(this, permissions)) {
            reqCode = Enums.RC_RC_BOOK_IMAGE
            ImageCropperUtils.launchImageCropperActivity(this)
        } else {
            getPermissionUtil().requestPermissions(this, permissions, Enums.FILE_REQ_CODE)
        }
    ...
    override fun onInsuranceClicked() {
        if (getPermissionUtil().hasPermission(this, permissions)) {
            reqCode = Enums.RC_INSURANCE_IMAGE
            ImageCropperUtils.launchImageCropperActivity(this)
        } else {
            getPermissionUtil().requestPermissions(this, permissions, Enums.FILE_REQ_CODE)
        }
