...
class FilterServiceListAdapter(val filterServiceListName: List<ConfigService>,
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        if (selectedPosition == position && selectedPosition != -1)
            holder.mBinding.filterserviceNameTv.setTextColor(
                    ContextCompat.getColor(context, R.color.colorBasePrimary)
            )
        else holder.mBinding.filterserviceNameTv.setTextColor(
